...
function calculateAtr(rows, period = config.atrPeriod) {
...
function calculateCmf(rows, period = config.cmfPeriod) {
...
function calculateObvSlope(rows, period = config.obvSlopeDays) {
  if (rows.length < period + 1) return null
  let obv = 0
  const values = []
  for (let i = rows.length - period; i < rows.length; i += 1) {
...
function computeSupportResistance(rows) {
  const slice = rows.slice(-config.srLookbackDays)
...
function intradaySignal(intradayRows) {
  if (!intradayRows?.length) return { prob: null, momentum: null, ret1: null, ret6: null, dayRet: null, vwapDist: null, timestamp: null }
...
function buildDailyRow(ticker, rows, benchmarkRows, sectorName, deliveryPct, intraday) {
  ...
  const w = rows.slice(-config.lookback52w)
  const high52 = Math.max(...highs(w))
  const low52 = Math.min(...lows(w))
  const distHigh = ((high52 - currentPrice) / high52) * 100
  const distLow = ((currentPrice - low52) / low52) * 100
  const nearHigh = distHigh <= config.nearPct52w
  const nearLow = distLow <= config.nearPct52w
  const avg20 = mean(volumes.slice(-20))
  const volRatio = avg20 ? latest.volume / avg20 : null
  const volSpike = volRatio !== null && volRatio >= config.volSpikeMult
  const volBreakout = volRatio !== null && volRatio >= config.volBreakoutMult
  const ma50 = rollingMean(closes, config.ma50, closes.length - 1)
  const ma200 = rollingMean(closes, config.ma200, closes.length - 1)
  ...
  const gapUp = gapUpPct !== null && gapUpPct >= config.gapUpPct
  ...
  const stopLoss = atr !== null ? currentPrice - config.atrStopMult * atr : null
  const riskPerShare = stopLoss !== null ? currentPrice - stopLoss : null
  const qty = riskPerShare && riskPerShare > 0 ? Math.floor(config.riskPerTradeInr / riskPerShare) : null
  const { support, resistance } = computeSupportResistance(rows)
  const cmf20 = calculateCmf(rows)
  const obvSlope = calculateObvSlope(rows)
  const rsi14 = calculateRsi(closes, 14)
  ...
  const accumulation = accumScore !== null && accumScore >= config.accumScoreMin && Boolean(above200dma)
  ...
  const aiPick = aiProb !== null && aiProb >= config.aiProbMin
  let score = 0
...
function buildDatasets(rows, liveRows, errors) {
...
async function generateScan(limit = null, persist = true, requestOptions = {}) {
  ...
  if (!tickers.length) {
    return buildDatasets([], [], [{ Ticker: null, Error: 'No watchlist symbols available from env, API, or fallback source.' }])
  }
  ...
  let rows = tickers.map((ticker) => {
    try {
      ...
      const intraday = intradaySignal(intradayMap[ticker] || [])
  ...
  const liveRows = Object.entries(intradayMap)
    .map(([ticker, intradayRows]) => {
      const signal = intradaySignal(intradayRows)
